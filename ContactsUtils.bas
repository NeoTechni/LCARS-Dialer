...
Sub Class_Globals
	...
	Private eventTypes As Map
	Private cr As ContentResolver
	Private dataUri As Uri, contactUri As Uri, rawContactUri As Uri
	
	'Type miniphone(PhoneNumber As String, SearchText As String)
	'Type minicontact(DisplayName As String, SearchText As String, Id As Long, Notes As String, PhoneNumbers As List, Starred As String)
	Public allItems As List
End Sub
...
Public Sub EnumContacts() As List 
	If Not(allItems.IsInitialized) Then
		allItems.Initialize 
		For Each c As cuContact In FindAllContacts(True)
			DoEvents
			allItems.Add(IDtoContact(c.id, c.DisplayName))
		Next
	End If
	Return allItems
End Sub

Public Sub IDtoContact(ID As Long, DisplayName As String) As Contact 
	Dim temp As Contact , CON As Contacts2 , tempContact As Contact , M As Map 
	If allItems.IsInitialized Then
		For Each C As Contact In allItems
...
'	temp.Id = ID
'	temp.GetEmails.Initialize ' map(addresses as keys, types as values)
'	temp.GetPhones.Initialize ' map(numbers as keys, types as values)
'	'temp.GetPhoto = GetPhoto(ID)
'	temp.LastTimeContacted = tempContact.LastTimeContacted  
'	temp.Name = temp.DisplayName 
'	temp.Notes = GetNote(ID)
'	temp.Starred = GetStarred(ID)
'	temp.TimesContacted = tempContact.TimesContacted 
'	temp.PhoneNumber = tempContact.PhoneNumber 
'	temp.GetPhones = tempContact.GetPhones 
'	temp.GetEmails = tempContact.GetEmails 
'	Return temp
End Sub
...
Public Sub FindContactsByPhone(PhoneNumber As String, Exact As Boolean, VisibleOnly As Boolean) As List
	Return FindContactsIdFromData("vnd.android.cursor.item/phone_v2", "data1", CleanPhone(PhoneNumber), "=", Exact, VisibleOnly)
End Sub
...
Private Sub FindContactsIdFromData (Mime As String, DataColumn As String, Value As String, Operator As String, 	Exact As Boolean, VisibleOnly As Boolean) As List
	If Not(Exact) Then 
		Operator = "LIKE"
		Value = "%" & Value & "%"
	End If
	...
	Dim crsr As Cursor = cr.Query(dataUri, Array As String("contact_id", "display_name"), selection, Array As String(Mime, Value), "")
	Dim res As List
	res.Initialize
	Dim m As Map
	m.Initialize
...
Public Sub GetEmails(Id As Long) As List
	...
	For Each obj() As Object In GetData("vnd.android.cursor.item/email_v2", Array As String("data1", "data2"), Id, Null)
		...
		e.Initialize
		e.Email = obj(0)
		e.EmailType = API.mailTypes.Get(obj(1))
		res.Add(e)
	Next
	Return res
End Sub
...
Public Sub GetPhones(id As Long) As List
	...
	For Each obj() As Object In GetData("vnd.android.cursor.item/phone_v2", Array As String("data1", "data2"), id, Null)
		...
		p.Initialize
		p.Number = obj(0)
		p.PhoneType = API.phoneTypes.Get(obj(1))
		res.Add(p)
	Next
	Return res
End Sub
...
Public Sub AddEmail(Id As Long, Email As String, EmailType As String)
	...
	If EmailType.Length=0 Then EmailType = 3
	v.PutInteger("data2", API.mailtypes.Get(EmailType))
...
Public Sub AddPhone(Id As Long, PhoneNumber As String, PhoneType As String)
	...
	If PhoneType.Length=0 Then PhoneType = 7
	v.PutInteger("data2", API.phoneTypes.Get(PhoneType))' GetKeyFromValue(API.phoneTypes, PhoneType, 7))
